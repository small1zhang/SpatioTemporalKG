...
class RuleEnforcer:
	...
	def __init__(self, rss_params=None, ego_config: Optional[EgoCentricConfig] = None):
		...
		self._ego_filter = (
			EgoCentricFilter(self._ego_config)
			if not self._ego_config.legacy_full_pairing
			else None
		)
	...
	def enforce(self, frame_id,
		...
		Args:
			ego_id: 可选显式自车 entity_id (为空时由 EgoCentricFilter 自动 pick).
			其它字段同前.
		...
		# 更新跨帧状态 (仍对全部 vehicles 累积, 用于 NoProperResponse 判定)
		self._update_state(vehicles, frame_id)

		# --- RSS 检查 ---
		# 阶段1: 默认走 ego × ROI 内他车 (legacy_full_pairing=True 退回原 O(N²))
		if self._ego_filter is None:
			self._rss_pair_scan_legacy(
				vehicles, frame_id,
				violations, violation_rels, defined_by_rels, responsibilities, resp_rels,
			)
		else:
			self._rss_pair_scan_ego_roi(
				vehicles, frame_id,
				violations, violation_rels, defined_by_rels, responsibilities, resp_rels,
			)
	...
	def _rss_pair_scan_legacy(
		...
		"""Legacy RSS 扫描: O(N²) 全对子 (向后兼容, legacy_full_pairing=True)."""
		n = len(vehicles)
		for i in range(n):
			v_a = vehicles[i]
			eid_a = v_a.get("entity_id", "")
			...
			for j in range(i + 1, n):
				...
				self._rss_check_one(
					v_a, v_b, eid_a, eid_b, speed_a, speed_b, frame_id,
					violations, violation_rels, defined_by_rels,
					responsibilities, resp_rels,
				)

	def _rss_pair_scan_ego_roi(
		self,
		vehicles: List[Dict[str, Any]],
		frame_id: int,
		violations: List[SafetyViolation],
		...
		if decision.ego is None:
			return
		ego = decision.ego
		eid_ego = ego.get("entity_id", "")
		speed_ego = ego.get("speed", 0.0)
		for v_b in decision.roi_targets:
			eid_b = v_b.get("entity_id", "")
			speed_b = v_b.get("speed", 0.0)
			self._rss_check_one(
				ego, v_b, eid_ego, eid_b, speed_ego, speed_b, frame_id,
				violations, violation_rels, defined_by_rels,
				responsibilities, resp_rels,
			)
	...
	def _rss_check_one(
		...
		"""对单个 (A, B) 对子运行 RSS 检查并产生违规节点/边.

		注意: brake_vals 取自 A 的 Eid, 用于 NoProperResponse 判定
		(原行为: legacy_html=A 是枚举中较小的索引).
		"""
		dx = v_a.get("location_x", 0) - v_b.get("location_x", 0)
		dy = v_a.get("location_y", 0) - v_b.get("location_y", 0)
		d_long = abs(dx)
