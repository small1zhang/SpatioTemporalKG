...
在 §2.4.3 / §3.3.3.1a / §3.3.3.2 / §4.4.2.1 / §6 表 6-6 联动新增 4 项 Mobileye RSS v3.0 [Mobileye, 2018] 扩充场景规则的形式化定义与公式表达，与 §3.3.3.1 基本 RSS 子层（R13a/R14a，源码已实现）形成"基本 + 扩充"双层结构。代码暂不实现，仅在论文中预留框架与接口。

| 修订项 | 章节 | 说明 |
|------|------|------|
| 新增 §3.3.3.1a "RSS 扩充场景规则" 子节 | `chapter3_03.md` | 含公式 (3.17a)-(3.17d) 与表 3-17a，四项扩充规则形式化定义 |
| 新增 §2.4.3 末尾 "RSS 扩充场景规则" 概览段 | `chapter2_04.md` | 引用 Mobileye v3.0 标准，对齐 §3.3.3.1a 编号 |
| §3.3.3.2 节标题更新 | `chapter3_03.md` | "交通法规子层与 RSS 扩充规则（R1–R18 + CUTIN/CUTOUT/NPR/CZ）" |
| §4.4.2.1 标题与说明更新 | `chapter4_04.md` | "RSS 残差向量" → "安全度量残差向量"，明确 5 维 = 2 RSS 公式 + 3 工程度量混合 |
| §6 表 6-6 主表 + 续表更新 | `chapter6_01.md` | 删除虚假 `RSS_R15a` 行，新增 4 条 RSS 扩充规则行（DR/FAR 暂记 — ），平均统计改为 13 条已实现规则；续表同步去 R15a |
| 注：4 条 RSS 扩充规则均标记为"框架式描述" | — | 公式形式与 Mobileye v3.0 [2018] §8.5.2/§8.5.3/§9.2/§10 兼容，代码可后续扩展 |

**4 项扩充规则编号**：`RSS_CUTIN` / `RSS_CUTOUT` / `RSS_NPR_ENH` / `RSS_CZ_ADAPT`，分别对应 Cut-in 切入安全距离、Cut-out 驶离缓冲、反应不当增强（NoProperResponse Refinement）、施工路段参数自适应。该 4 条规则保留 Mobileye v3.0 [2018] 标准的形式化定义，未触及源码（`stk/rules/rss/`），可在后续版本中按需逐步实现并补拍数据。

### 论文修订：审稿人视角第二轮深审修复（commit 1141d5a）

针对 RSS 公式与符号方向的全文统一性，进行第二轮审稿人视角深审，修复 4 项内部不一致：

| # | 章节 | 问题 | 修复方式 |
|---|------|------|---------|
| 1 | §3.3.3 公式 (3.13) | 横向安全距离缺 `max(0,…)` 包裹，与源码 `model.py:107` 不一致，理论上可能产生负值 | 加 `max(0,…)` 与源码/§2.4.3 (2.27) 严格一致 |
| 2 | §2.4.3 公式 (2.28) | 残差方向 `d - d_min` 与源码 `compute_kappa_rss` 中 `d_min - d` 反向 | 反转为 `d_min - d`，与源码/knowledge_injector/§4.4.2.1 全文统一为"危险程度正向"约定 |
| 3 | §4.4.2.1 公式 (4.24) | 残差方向 `d_min - d`（与源码一致）但表行文字"正常范围>0（安全）"与公式自相矛盾 | 表行更正为"正常范围 < 0（安全）"，与公式方向严格一致 |
| 4 | §3.3.3 节标题更新 | "交通法规子层（R1-R18）" → "交通法规子层与 RSS 扩充规则" | 与本章新增 §3.3.3.1a 扩充规则段配套 |

### 论文修订：RSS 公式三方严格对齐（commit de65994）

修复 §2.4.3 / §3.3.3 / §4.4.2.1 三章节中 RSS 公式表述的内部矛盾：

| 章节 | 修复 |
|------|------|
| §2.4.3 公式 (2.26) | 纵向公式补 4 项完整形式（含反应期加速项，原版缺） |
| §2.4.3 公式 (2.27) | 横向公式从车宽 `w_A/w_B` 变体 → Mobileye RSS v3.0 标准速度项版本 |
| §2.4.3 表 2-5 | 参数默认值同步 `DEFAULT_RSS_PARAMS` (ρ=0.3, a_min=3.0) |
| §3.3.3 公式 (3.11) | 加严格推导（四阶段运动学封闭表达论证） |
| §3.3.3 表 3-17 措辞 | "原论文表 1" → "Shalev-Shwartz 2017 与 Mobileye RSS v3.0"，澄清 ρ/a_min_brake 来源 |
| §4.4.2.1 表行 | `Eq.(3.6)` → `Eq.(3.12)`，`Eq.(3.8)` → `Eq.(3.14)` |

数值验证：纵向 d_min (v_A=10, v_B=8) = 16.19m；横向 (v_latA=1.5, v_latB=0.5) = 1.62m；源码 vs §3.3.3 vs §2.4.3 三方误差 0.000000m。
