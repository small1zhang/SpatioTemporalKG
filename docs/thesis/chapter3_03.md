...
该公式直接对应 Shalev-Shwartz 等 [Shalev-Shwartz, 2017] 表 1 与 Mobileye RSS v3.0 [Mobileye, 2018] 第 8.3 节的纵向安全距离闭式表达，由"反应期 + 制动期"两阶段最坏情形分析得到。设 $A$ 在 $t_0$ 时刻察觉危险，反应时间 $\rho$ 后开始制动，则反应期内 $A$ 行驶的距离上界由两项构成：

- 第一项 $v_A \rho$ 是 $A$ 在反应时间 $\rho$ 内维持当前速度 $v_A$ 匀速行驶的距离；
- 第二项 $\tfrac{1}{2} a_{\max,\text{accel}} \rho^2$ 是 $A$ 在反应期内以最大加速度 $a_{\max,\text{accel}}$ 加速所行驶的距离（防御后车在反应期有意加速的最坏情形，故 $a_{\max,\text{accel}}$ 称为"合理加速上界"而非实际加速度）。

反应期结束时 $A$ 的速度达到 $v_A' = v_A + a_{\max,\text{accel}} \rho$，此后 $A$ 进入制动期，以最低合理制动减速度 $a_{\min,\text{brake}}$ 刹车至完全停止：

- 第三项 $\frac{(v_A + a_{\max,\text{accel}}\rho)^2}{2\, a_{\min,\text{brake}}}$ 即为 $A$ 在制动期行驶的距离，使用最低合理制动减速度 $a_{\min,\text{brake}}$（保守上界，即"普通驾驶员在紧急制动时容易达到的最低水平"——若 $A$ 实际制动能力低于此，则安全距离无法保证）。

与此同时，前车 $B$ 在 $(\rho + \text{制动期})$ 全过程中也以最大合理减速度 $a_{\text{brake}}$ 制动，其行驶距离由第四项给出：

- 第四项 $-\frac{v_B^2}{2\, a_{\text{brake}}}$ 是前车 $B$ 在相同时间内制动至完全停止的距离，作为前车距离的削减量（前车也在减速，因此后车所需安全距离相应缩短）。

将反应期与制动期距离相加并扣除前车制动距离，即得式 (3.11)。该公式在 RSS 原论文中以**"最坏情形封闭表达"**形式给出，其严格性体现于：（i）四项均假设对方或自身采取最不利于安全的行为（后车在反应期加速、前车以最大减速度急刹），任何实际行为的危险程度均不会超过此封闭表达；（ii）只要 $A$ 实际保持 $d_{\text{long}}(A, B, t) \geq d_{\min}^{\text{long}}(A, B, t)$，则在数学上可证 $A$ 与 $B$ 不发生碰撞——该保证由运动学不等式严格推导，不依赖任何经验或统计数据。

参数值如表 3-17 所示。

**表 3-17** RSS 子层参数表（代码 `DEFAULT_RSS_PARAMS`，取值参考 Shalev-Shwartz 等 [Shalev-Shwartz, 2017] 与 Mobileye RSS v3.0 [Mobileye, 2018]）
...
参数来源包括：$\rho=0.3\text{ s}$、$a_{\min,\text{brake}}=3.0\text{ m/s²}$、$a_{\min,\text{lat,brake}}=1.5\text{ m/s²}$ 来自 Shalev-Shwartz 等 [Shalev-Shwartz, 2017] 表 1 与 Mobileye RSS v3.0 [Mobileye, 2018] 推荐参数（后者较前者更新了反应时间与纵向制动减速度的推荐值，将 $\rho$ 从 0.1 s 调整为 0.3 s、$a_{\min,\text{brake}}$ 从 5 m/s² 调整为 3 m/s²）；$a_{\max,\text{accel}}=0.5\text{ m/s²}$ 采用比两篇参考更保守的取值（原论文建议 1–2 m/s²），以优先保证安全距离计算的保守性——后车在反应期内即使有轻微加速企图，也不会缩小安全距离缺口；$a_{\text{brake}}=8.0\text{ m/s²}$ 是车辆在 ABS 系统辅助下的典型极限制动减速度，与前车极端急刹场景对应，在该取值下前车若以超过 $a_{\text{brake}}$ 的减速度急刹，则 RSS 责任归因自动判定责任在前车（符合"前车反常急刹不算后车责任"的物理直觉）。所有参数均可通过 `RuleEnforcer.__init__` 的 `rss_params` 动态覆盖，以适应不同安全严苛度场景。
...
$$
d_{\min}^{\text{lat}}(A, B, t) = \max\!\left(0,\ \mu + \frac{v_{\text{lat},A}^2}{2\,a_{\min,\text{lat,brake}}} + \rho\,v_{\text{lat},A} - \frac{v_{\text{lat},B}^2}{2\,a_{\min,\text{lat,brake}}}\right)
\tag{3.13}
$$

横向安全距离公式与纵向类似但增加了基础安全裕度 $\mu=0.5$ m，对应车辆侧向的最小物理缓冲间距（车门间距、车身晃动等）。横向场景下两车的"互相逼近"由各自的横向速度分量驱动，公式结构与纵向对称：第一项 $\mu$ 是常数裕度；第二项是 $A$ 车的横向刹车距离；第三项 $\rho v_{\text{lat},A}$ 是反应期内 $A$ 车横向惯性位移；第四项 $-\frac{v_{\text{lat},B}^2}{2 a_{\min,\text{lat,brake}}}$ 是 $B$ 车横向刹车距离的削减项。外层 $\max(0,\cdot)$ 是必要的物理非负约束——当对方车 $B$ 的横向速度 $v_{\text{lat},B}$ 远大于自车 $v_{\text{lat},A}$ 时（例如 $B$ 主动远离 $A$ 的方向高速横移），第四项削减量会超过前三项之和，导致 $d_{\min}^{\text{lat}}$ 计算为负；此时由于 $B$ 实际在远离 $A$，不存在横向碰撞风险，最小安全距离按 0 取（即仅保留常数裕度 $\mu$ 已被外层 $\max$ 截断至 0），物理意义上合理。源码 `compute_dmin_lat` 实现中亦保留此 $\max(0,\cdot)$ 截断以保证安全距离始终非负。
